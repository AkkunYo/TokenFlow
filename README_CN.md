...
**集成 CLIProxyAPI、gemflow 粘滞负载网关与 Mihomo 多出口路由的统一高可用聚合网关。**
...
```text
apps/tokenflow/       CPA、NVIDIA 与产品启动编排
...
```text
                 ...
                 [TokenFlow / CLIProxyAPI :18317]
                       │                     │
                       ▼                     ▼
              [gemflow 粘滞网关]       [NVIDIA Provider Keys]
                    :8081               逐 Key proxy-url
                       │                     │
                       ▼                     │
                 [Workers 1..N]              │
                       │                     │
...
一键脚本自动检测系统架构 (`amd64`/`arm64`)，安装 Mihomo、CLIProxyAPI 和 Python 运行依赖，并配置好 **systemd 系统服务**守护。
...
services:
  tokenflow:
    image: ghcr.io/akkunyo/tokenflow:latest
